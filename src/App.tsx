...
function CertificationSection() {
  ...
  return (
    <section id="certification" className="bg-white u-section-sm scroll-mt-16 lg:scroll-mt-24">
      <div className={SHELL}>
        <div className="max-w-[680px] mx-auto text-center flex flex-col items-center gap-3" data-reveal="up">
          <span
...
// reference(touchagraphic.com about-us .about-sec03)의 구조적 DNA(OUR PARTNER naming,
// Institution/Brand grouping, horizontal logo flow, thin divider, 절제된 로고 표현)만 가져오고
// exact 수치(104px title, 모든 pill)는 V2 전체 hierarchy에 맞춰 다시 잡는다(§완료보고).
// OUR PARTNER.는 Hero h1(최대 80px, 위 참고)을 넘지 않는 선에서 이 페이지의 유일한 대형 제목,
// Institution/Brand는 그 아래 category label로 명확히 낮은 위계를 갖는다.
function Clients() {
  return (
  ...
  return (
    <section id="faq" className="bg-black text-white pt-[64px] lg:pt-[112px] pb-[100px] lg:pb-[160px] scroll-mt-24">
      ...
      <div className={SHELL}>
        ...
        <div className="flex items-center gap-2.5 mb-5" data-reveal="up">
          <span className="w-1.5 h-1.5 rounded-full" style={{ background: '#FF2D16' }} aria-hidden />
...
export default function App() {
  ...
  // route가 바뀌어 다른 페이지 트리가 마운트될 때마다 새로 그려진 [data-reveal] 요소를
  // 다시 관찰한다(§완료보고 — App()은 하나의 컴포넌트라 hook은 조건부 return 이전에 호출).
